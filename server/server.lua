	...
	Tip4serv.check_pending_commands = function (server_id,private_key,public_key,timestamp)
		...
		PerformHttpRequest(statusUrl, function(statusCode, tip4serv_response, _)
			...
			for k,infos in ipairs(json_decoded) do
				...
				--Check if player is online and get xPlayer ID
				Player_id = Tip4serv.checkifPlayerIsOnline(infos)	
				xplayerid = infos["xplayerid"]
				if Player_id then
					...
					if ESX then
						if xplayerid == "" then
							...
							if xPlayer then
								xplayerid = xPlayer.identifier
								new_obj["xplayerid"] = xplayerid
							else
								Player_id = false
							end
						end
				...
				for k,cmd in ipairs(infos["cmds"]) do
					...
					if (Player_id == false and (string.match(cmd["str"], "{") or cmd["state"] == 1)) then
						...
						if (xplayerid and string.match(cmd["str"], "{fivem_xplayer_id}")) then
							cmd["str"] = string.gsub(cmd["str"], "{fivem_xplayer_id}", xplayerid)
						end
...
Citizen.CreateThread(function()
	while true do
		key_arr = {} i = 0
		for info in string.gmatch(tip4serv_key, '([^.]+)') do key_arr[i] = info i = i+1 end
		if (i ~= 3) then
			print("^1[Tip4serv error] Please set tip4serv_key to a valid key in tip4serv/server/server.lua then restart tip4serv resource. Make sure you have copied the entire key on Tip4serv.com (CTRL+A then CTRL+C)^7")
			CancelEvent() return
		end
